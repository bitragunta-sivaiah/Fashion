...
const Register = () => {
  ...
  const { loading, error, user } = useSelector((state) => state.auth);
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        // make a API call to register user
        dispatch(registerUser(data));
    };

    useEffect(() => {
        if (user) {
          navigate('/login'); // Navigate to the dashboard or any protected route after login
        }
      }, [user, navigate]);
        ...
        <div  className='w-[100vw] h-[100vh] fixed top-0 left-0 bg-gray-50 flex items-center justify-center'>
            <div className="w-full max-w-xl p-3 relative bg-white shadow rounded-lg">
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    <button 
                        type="submit" 
                        className="w-full px-4 cursor-pointer py-2 text-[20px] text-white bg-blue-500 rounded-md hover:bg-blue-700"
                    >
                        Register
                    </button>
                </form>
